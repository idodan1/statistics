...
knitr::include_graphics("updated logo page.png")
```
...
labels[3] <- paste0("Non ST elevation MI (Non STEMI) \n n = ",table(dat$S21DISDIA_new%in%c("NSTEMI", "Thromboembolic", "UAP", "MINOCA - VASOSPASTIC,","Microvascular"))["TRUE"]," (",round(100*(table(dat$S21DISDIA_new%in%c("NSTEMI", "Thromboembolic", "UAP", "MINOCA - VASOSPASTIC,","Microvascular" ))["TRUE"]/nrow(dat)),1),"%)")
...
Prior to the index hospitalization, a higher proportion of patients with non STEMI (`r round(100*prop.table(table(dat$S21C1, dat$ST_ELEV),2),1)["YES","No"]`%) were being treated with aspirin compared to those with STEMI
(`r round(100*prop.table(table(dat$S21C1, dat$ST_ELEV),2),1)["YES","Yes"]`%). Other drugs in common use were Angiotensin-Converting-Enzyme (ACE) Inhibitors and Angiotensin Receptor Blockers (ARB), Beta Blockers,
lipid-lowering drugs (primarily statins) and diuretics all of which were in
use more frequently among patients presenting with non STEMI. `r round(100*prop.table(table(dat$S21C2, dat$ST_ELEV),2),1)["YES","No"]`% of
patients with non STEMI and `r round(100*prop.table(table(dat$S21C2, dat$ST_ELEV),2),1)["YES","Yes"]`% of those with STEMI were being
...
tab_trans_st <- print(CreateTableOne(vars = "S21TMODE", strata = "ST_ELEV_adm", data = droplevels(dat[!is.na(dat$S21TMODE) & dat$S21TMODE!="Not relevant (e.g. in-patient)",])))
...
```{r results='hide'}
dat$S21FIRST <- factor(ifelse(dat$S21FIRST=="Directly to CCU", "Directly to ICCU", as.character(dat$S21FIRST))) 
tab_first <- print(CreateTableOne(vars = "S21FIRST", data = dat))
tab_first_st <- print(CreateTableOne(vars = "S21FIRST", strata = "ST_ELEV_adm", data = dat))
...
tab_micu_st <- print(CreateTableOne(vars = "S21FIRST", strata = "ST_ELEV_adm", data = subset(dat, S21TMODE=="Mobile ICCU")))
...
in the ICCU (`r round(100*prop.table(table(dat$S21WARD1, dat$ST_ELEV_adm),2),1)["CCU","Yes"]`%). `r round(100*prop.table(table(dat$S21WARD1, dat$ST_ELEV_adm),2),1)["CCU","No"]`% of the patients who presented
with non ST elevation were admitted to the ICCU and an additional `r round(100*prop.table(table(dat$S21WARD1, dat$ST_ELEV_adm),2),1)["Cardiology","No"]`% to a
cardiology department, with the remaining `r round(100*prop.table(table(dat$S21WARD1, dat$ST_ELEV_adm),2),1)["Internal medicine","No"]`% being admitted to internal
...
```{r results='hide'}
dat$S21WARD1 <- factor(ifelse(dat$S21WARD1=="CCU","ICCU", as.character(dat$S21WARD1)), levels = c(  "ICCU","Cardiology" ,"Internal medicine" , "Chest pain unit","Other"  ))
tab_Ihosp <- print(CreateTableOne(vars = "S21WARD1", data = dat))
tab_Ihosp_st <- print(CreateTableOne(vars = "S21WARD1", strata = "ST_ELEV_adm", data = dat))
...
tab_heft_st <- print(CreateTableOne(vars = vars_heft, strata = "ST_ELEV_adm", data = dat[!(dat$S21MEASAT %in% c("ED","In-patient")),]), nonnormal = vars_heft)
...
`r round(100*prop.table(table(dat$S21MEASAT)),1)["ED"]`% of patients had the first medical contact at the ED and about `r round(100*prop.table(table(dat$S21MEASAT)),1)[2]`% at a Health maintenance organization (HMO) primary clinic/"Moked". For an additional `r round(100*prop.table(table(dat$S21MEASAT)),1)["Mobile ICCU"]`% the primary medical contact was with MICU.
Patients with ST elevation were more likely to have their first medical contact with a MICU (`r round(100*prop.table(table(dat$S21MEASAT, dat$ST_ELEV_adm),2),1)["Mobile ICCU","Yes"]`%) than those with non ST elevation (`r round(100*prop.table(table(dat$S21MEASAT, dat$ST_ELEV_adm),2),1)["Mobile ICCU","No"]`%).
...
tab_fmed_st <- print(CreateTableOne(vars = "S21MEASAT", strata = "ST_ELEV_adm", data = dat))
...
Typical angina was significantly more frequent in patients presenting with ST elevation (`r round(100*prop.table(table(dat$S21ADMSY1, dat$ST_ELEV_adm),2),1)["YES","Yes"]`%) than those presenting with non ST elevation (`r round(100*prop.table(table(dat$S21ADMSY1, dat$ST_ELEV_adm),2),1)["YES","No"]`%). However, atypical chest pain was more common in patients presenting with non ST elevation (`r round(100*prop.table(table(dat$S21ADMSY3, dat$ST_ELEV_adm),2),1)["YES","No"]`%) than in those with ST elevation (`r round(100*prop.table(table(dat$S21ADMSY3, dat$ST_ELEV_adm),2),1)["YES","Yes"]`%).
Also dyspnea was more common in patients with non ST elevation (`r round(100*prop.table(table(dat$S21ADMSY2, dat$ST_ELEV_adm),2),1)["YES","No"]`%) than those with ST elevation (`r round(100*prop.table(table(dat$S21ADMSY2, dat$ST_ELEV_adm),2),1)["YES","Yes"]`%).
...
tab_killip <- print(CreateTableOne(vars = "S21ADMKLP", strata = "ST_ELEV_adm", data = dat, factorVars = "S21ADMKLP"))
...
tab_bhosp_tr_st <- print(CreateTableOne(vars = vars_bhosp_tr, strata = "ST_ELEV_adm", data = subset(dat, S21TMODE %in% c("Mobile ICCU","Regular ambulance"))))
...
`r round(100*prop.table(table(dat$S21ECGLOC, dat$ST_ELEV_adm),2),1)["ED","No"]`% of patients presenting with non ST elevation and `r round(100*prop.table(table(dat$S21ECGLOC, dat$ST_ELEV_adm),2),1)["ED","Yes"]`% of patients
presenting with ST elevation had their first ECG recorded in the emergency department (ED).
With respect to the remaining patients, `r round(100*prop.table(table(dat$S21ECGLOC, dat$ST_ELEV_adm),2),1)["Home","Yes"]+round(100*prop.table(table(dat$S21ECGLOC, dat$ST_ELEV_adm),2),1)["Ambulance","Yes"]`% of patients with ST elevation and
`r round(100*prop.table(table(dat$S21ECGLOC, dat$ST_ELEV_adm),2),1)["Home","No"]+round(100*prop.table(table(dat$S21ECGLOC, dat$ST_ELEV_adm),2),1)["Ambulance","No"]`% of those with non ST elevation had the first ECG performed either at home or in
...
```{r fig.width=8}
par(mar=c(4,4,4,2))
t_ecgloc <- addmargins(table(dat$S21ECGLOC, dat$ST_ELEV_adm))
...
About `r round(100*prop.table(table(dat$S21ECGRHY)))["NSR"]`%  of patients presented with a normal sinus rhythm (NSR). `r round(100*prop.table(table(dat$S21ECGRHY, dat$ST_ELEV_adm),2),1)["AF","Yes"]`% of patients with ST elevation and `r round(100*prop.table(table(dat$S21ECGRHY, dat$ST_ELEV_adm),2),1)["AF","No"]`% of those without ST elevation, presented with atrial fibrillation.
...
tab_ecgrhy_st <- print(CreateTableOne(vars = "S21ECGRHY", strata = "ST_ELEV_adm", data = dat))
...
`r round(100*(table(dat$ST_ELEV,dat$S21REP)[2,2])/table(dat$ST_ELEV)[2],1)`% of patients with STEMI underwent primary reperfusion within 12 hours from onset of symptoms, mainly primary PCI. In `r round(100*(table(dat_st_rep$S21PCISTENT,dat_st_rep$REP=="PRIMARY PCI")[2,2])/table(dat_st_rep$REP=="PRIMARY PCI")[2],1)`% of these cases, stents were deployed. Of the remaining `r 100-(round(100*(table(dat$ST_ELEV,dat$S21REP)[2,2])/table(dat$ST_ELEV)[2],1))``% which did not undergo primary reperfusion, `r round(100*(table(dat_st$S21REP=="No" & dat_st$S21ANGIO=="Yes")["TRUE"]/table(dat_st$S21REP=="No")["TRUE"]),1)`% eventually underwent coronary angiography. Of these, `r round(round(100*(table(dat_st$S21REP=="No" & dat_st$S21ANGIO=="Yes" & dat_st$S21HPCI=="Yes")["TRUE"]/table(dat_st$S21REP=="No" & dat_st$S21ANGIO=="Yes")["TRUE"]),1) + round(100*(table(dat_st$S21REP=="No" & dat_st$S21ANGIO=="Yes" & dat_st$S21HCABG=="Yes")["TRUE"]/table(dat_st$S21REP=="No" & dat_st$S21ANGIO=="Yes")["TRUE"]),1))`% underwent revascularization.
...
n_arr_rep_w <- c(#length(na.omit(dat[dat$S21SEX=="Female","HEFT9"])),
                 length(na.omit(dat[dat$S21SEX=="Female","ARR_TLX"])),length(na.omit(dat[dat$S21SEX=="Female","ARR_PPCI"])))
n_arr_rep_m <- c(#length(na.omit(dat[dat$S21SEX=="Male","HEFT9"])),
                 length(na.omit(dat[dat$S21SEX=="Male","ARR_TLX"])),length(na.omit(dat[dat$S21SEX=="Male","ARR_PPCI"])))
...
mydata_arr_rep_u_w <- as.numeric(substr(t_arr_rep_gen[,1],13,18))
...
mydata_arr_rep_m <- as.numeric(substr(t_arr_rep_gen[,3],1,4))
mydata_arr_rep_l_m <- as.numeric(c(substr(t_arr_rep_gen[1,3],8,10),
                                 substr(t_arr_rep_gen[2,3],6,7))
                                 )
mydata_arr_rep_u_m <- as.numeric(c(substr(t_arr_rep_gen[1,3],13,17),
                                 substr(t_arr_rep_gen[2,3],11,14))
                                 )
...
Ejection fraction (EF) was determined in `r round(100*prop.table(table(dat$S21HOSEF, dat$ST_ELEV),2),1)["Yes","Yes"]`% of patients with STEMI and in
`r round(100*prop.table(table(dat$S21HOSEF, dat$ST_ELEV),2),1)["Yes","No"]`% of those with non STEMI. EF was normal in a larger proportion of patients
with non STEMI (`r round(100*prop.table(table(dat$S21HOSEFR, dat$ST_ELEV),2),1)["Normal (55-65%)","No"]`%) than in patients with STEMI (`r round(100*prop.table(table(dat$S21HOSEFR, dat$ST_ELEV),2),1)["Normal (55-65%)","Yes"]`%). `r round(100*prop.table(table(dat$S21HOSEFR, dat$ST_ELEV),2),1)["Severe (< 30%)","Yes"]+round(100*prop.table(table(dat$S21HOSEFR, dat$ST_ELEV),2),1)["Moderate (30-39%)","Yes"]`% of
patients with STEMI and `r round(100*prop.table(table(dat$S21HOSEFR, dat$ST_ELEV),2),1)["Severe (< 30%)","No"]+round(100*prop.table(table(dat$S21HOSEFR, dat$ST_ELEV),2),1)["Moderate (30-39%)","No"]`% of patients with non STEMI presented with an EF < 40%.
...
dat_surv$S21DISTO <- factor(ifelse(dat_surv$S21DISTO=="Recovery hotel (Convalescence facility)", "Home", as.character(dat_surv$S21DISTO)), levels = c( "Home",
                                                                                                                                                       "Internal medicine"  ,
...
```{r results='hide'}
vars_out <- c("S21DISDIE","DIED7","DIED30","MACE_OLD", "DIED365") 
...
m4f <- glm(DIED365~ST_ELEV+AGE+sex+S21PRIOR_ACS+S21PDIAB+S21HT+KILIP234+ANGIO, family = "binomial", data = dat)
...
ms4f <- glm(DIED365~sex+AGE+S21PRIOR_ACS+S21PDIAB+S21HT+KILIP234+ANGIO, family = "binomial", data = dat)
...
This is the first time we performed 90 days follow up survey. We performed this survey in order to evaluate patient’s adherence to treatment and life-style changes recommendations.
...
Ninety-day follow-up was performed for `r nrow(dat_fu)` (`r round((nrow(dat_fu) / nrow(dat))*100)`%) patients. Of which `r table(dat_fu$S21FU_CONTACT)["Telephone"]` (`r  round(100*prop.table(table(dat_fu$S21FU_CONTACT))["Telephone"])`%) were contacted by phone, `r table(dat_fu$S21FU_CONTACT)["Clinic visit"]` (`r  round(100*prop.table(table(dat_fu$S21FU_CONTACT))["Clinic visit"])`%) by clinical visits and `r table(dat_fu$S21FU_CONTACT)["Virtual communication"]` (`r  round(100*prop.table(table(dat_fu$S21FU_CONTACT))["Virtual communication"])`%) by virtual communication. Most of the patients were asymptomatic and in NYHA Class I.
...
```{r, results='hide'}
vars_med <- c(
"S21F1"	#FU 90D TREAT. Aspirin
,"S21F2"	#FU 90D TREAT. Clopidogrel
,"S21F3"	#FU 90D TREAT. Prasugrel
,"S21F4"	#FU 90D TREAT. Ticagrelor
,"S21F9"	#FU 90D TREAT. Eliquis
,"S21F7"	#FU 90D TREAT. Pradaxa
,"S21F8"	#FU 90D TREAT. Xarelto 90 days
,"S21F6"	#FU 90D TREAT. Coumadin
,"S21F60"	#FU 90D TREAT. Clexane
,"S21F14"	#FU 90D TREAT. ACE-I
,"S21F15"	#FU 90D TREAT. ARB's
,"S21F61"	#FU 90D TREAT. ARNI
,"S21F16"	#FU 90D TREAT. Aldactone
,"S21F17"	#FU 90D TREAT. Beta blockers
,"S21F18"	#FU 90D TREAT. Digoxin
,"S21F19"	#FU 90D TREAT. CCB
# ,"S21F20"	#FU 90D TREAT. Amiodarone
# ,"S21F21"	#FU 90D TREAT. Other anti-arrhythmic
# ,"S21F22"	#FU 90D TREAT. Nitrates
,"S21F23"	#FU 90D TREAT. Diuretics
,"S21F24"	#FU 90D TREAT.PPI's
# ,"S21F25"	#FU 90D TREAT. H2 blockers
# ,"S21F26"	#FU 90D TREAT. NSAIDS
# ,"S21F27"	#FU 90D TREAT. Colchine
# ,"S21F28"	#FU 90D TREAT. Steroids
,"FU_STAT"
,"S21F58"	#FU 90D TREAT. Dapagliflozin (Farxiga) for non diabetic
,"S21F59"	#FU 90D TREAT. Empagliflozin (Jardiance) for non diabetic
...
vars_med <- c(
 "S21F33",	#FU 90D TREAT. Insulin SC
"S21F34",	#FU 90D TREAT. Glibenclamide (Gluben)
"S21F35",	#FU 90D TREAT. Glipizide (Gluco-Rite)
"S21F36",	#FU 90D TREAT. Glimepiride (Amaryl)
"S21F37",	#FU 90D TREAT. Metformin (Glucophage)
"S21F38",	#FU 90D TREAT. Sitagliptine (Januvia)
"S21F39",	#FU 90D TREAT. Saxagliptine (Onglyza)
"S21F40",	#FU 90D TREAT. Vidagliptine (Galvus)
"S21F41",	#FU 90D TREAT. Linagliptine (Trajenta)
"S21F42",	#FU 90D TREAT. Exenatide (Byetta, Budyreon)
"S21F43",	#FU 90D TREAT. Liraglutide (Victoza)
"S21F62", #Dulaglutide (Trulicity)
"S21F57", #Semaglutide (Ozempic)
"S21F44",	#FU 90D TREAT. Dapagliflozin (Farxiga)
"S21F56",	#FU 90D TREAT. Empagliflozin (Jardiance)
"S21F45",	#FU 90D TREAT. Acrabose (Prandase)
"S21F46",	#FU 90D TREAT. Meglinitides (Repaglinide, Novonorm)
"S21F47"	#FU 90D TREAT. TZDs (Pioglitasone - actos, Rosiglitazone - Avandia)

)

dat$S21F39 <- factor(dat$S21F39, levels = c("NO","YES"))
dat$S21F42 <- factor(dat$S21F42, levels = c("NO","YES"))
...
tab1 <- print(CreateTableOne(vars = vars_med,data = subset(dat, S21PDIAB=="Yes")), missing=F, printToggle=F)
rownames(tab1) <- c("n", paste0(vars_med_names, " (%)"))
...
vars_lab <- c(
 "S21FCHO"	#Cholesterol - total 90 days (mg/dL or mg%)
 ,"S21FLDL"	#Cholesterol - LDL 90 days (mg/dL or mg%)
 ,"S21FHDL"	#Cholesterol - HDL 90 days (mg/dL or mg%)
 ,"S21FTG"	#Triglycerides 90 days (mg/dL or mg%)
 ,"S21FGLU"	#Glucose 90 days (mg/dL)
 ,"S21FHB"	#HB 90 days (g/dL or g%)
 ,"S21FCREV"	#Creatinine 90 days (mg/dL)
 # ,"S21FPLT"	#Platelets 90 days (10e3/microL)
 ,"S21FHBA1C"	#HbA1C 90 days (%)
 # ,"S21FALBUM"#Albumin 90 days (g/dL)
)
...
Concerning life-style modification, `r round(100*(prop.table(table2(dat_fu$S21FREGULAR_EXERCISE))["Yes"]))`% of patients reported to perform regular weekly exercise and `r round(100*(prop.table(table2(dat_fu$S21FDIET))["Modified"]))`% patients reported about diet change. Smoking cessation was reported in `r table2(dat_fu[dat_fu$CURSMOK=="YES" & !is.na(dat_fu$S21FSMK),]$S21FSMK)["quit smoking"]` (`r round(100*(prop.table(table2(dat_fu[dat_fu$CURSMOK=="YES" & !is.na(dat_fu$S21FSMK),]$S21FSMK))["quit smoking"]))`%) of the patients who were active smokers during the index hospitalization.

Despite the recommendation for cardiac rehabilitation programs, only `r round(prop.table(table2(dat_fu$S21FREHP))["Yes"]*100 + prop.table(table2(dat_fu$S21FREHP))["Scheduled to participate"]*100)`% of patients were actively participating or scheduled.
...
source('C:/Users/talco/Documents/Statistics/ACSIS_21_PDF/Data_ACSIS_Trends_00_21.R')
```
...
row.names(t2.11) <- c("n","Aspirin ($\\%$)","Beta Blockers ($\\%$)","P2Y12 inhibitors ($\\%$)",
             ...
             ,paste0("Anticoagulant", footnote_marker_number(2), " ($\\%$)")
