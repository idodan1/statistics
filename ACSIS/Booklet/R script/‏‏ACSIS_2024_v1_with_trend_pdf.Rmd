...
# Function to increment and retrieve figure number
next_figure <- function(sub = FALSE) {
  if (sub) {
    # For sub-figures like a, b, c
    figure_num_sub <<- figure_num_sub + 1
    return(paste0(figure_num, ".", letters[figure_num_sub]))
  } else {
    figure_num <<- figure_num + 1
    figure_num_sub <<- 0
    return(as.character(figure_num))
  }
}

# Function to increment and retrieve table number
next_table <- function() {
  table_num <<- table_num + 1
  return(as.character(table_num))
}

# Function to increment and retrieve section number
next_section <- function() {
  section_num <<- section_num + 1
  return(paste0(chapter_num, ".", section_num))
}

# Initialize sub-figure counter
figure_num_sub <- 1
```
...
# tab_norep = transform_tableone(tab_norep)
t_norep <- tab_norep[-1]
```
...
dat$CCUDAYS - dat$ISHDAYS
dat$ISHDAYS[1:20]
mean(dat$ISHDAYS, na.rm=TRUE)
mean(dat$CCUDAYS, na.rm=TRUE)
