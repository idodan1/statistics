...
transform_tableone <- function(table_one) {
  ...
  for (i in seq_len(nrow(mat))) {
    ...
    if (grepl("= NO (%)", rownames(mat)[i])) {
      ...
      no_percent <- as.numeric(gsub("[() %]", "", mat[i, 1]))
      total <- as.numeric(gsub("[() %]", "", mat[i - 1, 1]))
      
      # Calculate YES percentage
      yes_percent <- 100 - no_percent
      
      # Replace the 'NO' row with 'YES' and the calculated percentage
      rownames(mat)[i] <- gsub("NO", "YES", rownames(mat)[i])
      mat[i, 1] <- paste0(total, " ( ", yes_percent, ")")
    }
  }
}
```
